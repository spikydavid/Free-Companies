...
import { getSession, rerollManualBattleSession } from "@/lib/field-of-honour/session-store";
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    const body = (await request.json().catch(() => ({}))) as RerollBody;
    ...
    const battle = rerollManualBattleSession(sessionId, body.type, body.index);
    return NextResponse.json({ sessionId, battle });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to reroll die" },
      { status: 400 },
    );
  }
}
